...
public class DiemThiSubPanel extends JPanel {
    ...
    public DiemThiSubPanel() {
        ...
        statsPanel.add(lblCount); statsPanel.add(lblToanStats); statsPanel.add(lblVanStats);
        statsPanel.add(lblAnhStats); statsPanel.add(lblLyStats); statsPanel.add(lblHoaStats);
        centerPanel.add(statsPanel, BorderLayout.NORTH);

        // Bảng dữ liệu - 18 cột
        String[] header = {"CCCD", "SBD", "PT", "Toán", "Văn", "Anh", "Lý", "Hóa", "Sinh", "Sử", "Địa", "ĐGNL", "CNCN", "CNNN", "Tin", "KTPL", "NK1", "NK2"};
        ...
        for (DiemThiXettuyen d : listDiemThi) {
            ...
            if (match) {
                tableModel.addRow(new Object[]{
                    d.getCccd(), d.getSoBaoDanh(), d.getDPhuongThuc(),
                    f(d.getTo()), f(d.getVa()), f(d.getN1Thi()), f(d.getLi()), f(d.getHo()), 
                    f(d.getSi()), f(d.getSu()), f(d.getDi()), f(d.getNl1()),
                    f(d.getCncn()), f(d.getCnnn()), f(d.getTi()), f(d.getKtpl()), f(d.getNk1()), f(d.getNk2())
                });
                tT += (d.getTo()!=null?d.getTo():0); tV += (d.getVa()!=null?d.getVa():0);
                tA += (d.getN1Thi()!=null?d.getN1Thi():0); tL += (d.getLi()!=null?d.getLi():0);
    ...
    private void handleEdit() {
        ...
        if (r == -1) { JOptionPane.showMessageDialog(this, "Chọn dòng để sửa!"); return; }
    ...
    private void handleDelete() {
        ...
        if (r == -1) return;
        DiemThiXettuyen dt = listDiemThi.get(r);
        if (JOptionPane.showConfirmDialog(this, "Xóa " + dt.getCccd() + "?") == JOptionPane.YES_OPTION) {
            DAOFactory.getDiemThiDAO().delete(dt);
            DiemPanel.addLog("Xóa điểm CCCD: " + dt.getCccd());
            loadData();
        }
    }
    ...
    private void handleImport() {
        ...
        if (fs.showOpenDialog(this) == JFileChooser.APPROVE_OPTION) {
            ...
            for (DiemThiXettuyen dt : list) {
                ...
                if(ex.isEmpty()) DAOFactory.getDiemThiDAO().save(dt); else DAOFactory.getDiemThiDAO().update(dt);
            }
            loadData();
            JOptionPane.showMessageDialog(this, "Nhập Excel hoàn tất!");
        }
    }
